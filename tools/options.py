...
Documentation may be found in docs/Toolchain_Profiles.md"""


def get_default_options_parser(add_clean=True, add_options=True, add_app_config=False):
...
def list_profiles():
    ...
    Checks default profile directory (mbed-os/tools/profiles/) for all the json files
    and return list of names only
    """
    return [
        fn.replace(".json", "")
        for fn in listdir(join(dirname(__file__), "profiles"))
        if fn.endswith(".json")
    ]
...
def extract_profile(parser, options, toolchain, fallback="develop"):
    ...
    for filename in filenames:
        ...
        if toolchain not in contents:
            fmt_str = "argument --profile: toolchain {} is not" " supported by profile {}"
            msg = fmt_str.format(toolchain, filename)
            args_error(parser, msg)
