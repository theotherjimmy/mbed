...
class mbedToolchain:
    ...
    def add_linker_defines(self):
        ...
        if self.target.is_PSA_non_secure_target:
            flags2params = {
                "MBED_APP_START": "target.non-secure-flash-start",
                "MBED_APP_SIZE": "target.non-secure-flash-size",
                "MBED_RAM_START": "target.non-secure-ram-start",
                "MBED_RAM_SIZE": "target.non-secure-ram-size"
            }
        if self.target.is_PSA_secure_target:
            flags2params = {
                "MBED_APP_START": "target.secure-flash-start",
                "MBED_APP_SIZE": "target.secure-flash-size",
